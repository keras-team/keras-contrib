...
def get_wts(h5_file, layer_name, identifier):
    ...
    dataset_path = 'model_weights/' + layer_name + '/' + layer_name + '/' + identifier + ':0'
...
def scale_layers(wts, biases, next_weights):
    ...
    b_norm = np.asarray([b/wts_sqrt[index] for index, b in enumerate(biases)])
...
def reduce_layers(layer_curr_weights, layer_curr_biases, layer_next_scaled, cc_list):
    #reducing the weight filters
    layer_curr_filter_groups = [layer_curr_weights[x] for x in cc_list]
    layer_curr_filter_reduced = [x.mean(axis=0) for x in layer_curr_filter_groups]
...
def extract_current_and_next_weights(model, op_layer, next_layer):
    ...
    #print("required 4D tensor shapes (for 2D conv) --> ", no_filters, no_input_channels, kernel_w, kernel_h)
...
def modify_model_arch(json_mod_pack, model_mod_pack):
    ...
    new_params = modify_model_params(model_params_file, current_layer, next_layer, new_weights, new_biases, new_next_layer_weights, make_copy)
                                    ...
                                    if layer_key=='config':
                                        for a,b in layer_val.items():
                                            if layer_val['name']==current_layer and a=='filters':
                                                layer_val[a] = reduced_number_filters
                                                break
...
def modify_model_params(model_params_file, current_layer, next_layer, new_weights, new_biases, new_next_layer_weights, make_copy=0):
    """
    ...
    if make_copy==1:
        ...
        h5repack_cmd = 'h5repack -i' +  model_params_file + '-o' + new_model_params_file
        os.system(h5repack_cmd)
        op_h5file = new_model_params_file
        print("made a copy of the existing model params with the name {}".format(new_model_params_file))
    ...
    if existing_params['wts'].shape == new_weights.shape:
        print("Shape of the new weight matrix is same as the shape of current weight matrix. Use modify_sameShape_model_params() instead")
        return
...
def modify_sameShape_model_params(model_params_file, current_layer, next_layer, new_weights, new_biases, new_next_layer_weights, make_copy=0):
    op_h5file = model_params_file
    if make_copy==1:
    ...
    existing_params = extract_existing_params(op_h5file, path_dict)
    if existing_params['wts'].shape != new_weights.shape:
        print("Shape of the new weight matrix is different from the shape of current weight matrix. Use modify_model_params() instead")
        return
    modify_h5_params(op_h5file, path_dict['wts'], new_weights)
    # print("current layer existing weights shape: ", existing_params['wts'].shape)
    # print("current layer new weights shape: ", )
...
def modify_batchnorm(op_h5file, path_dict, new_shape, existing_params):
    ...
    for param in bn_path_list:
        ...
        if path in fh.keys():
            ...
            del fh[path]
            fh.create_dataset(path, data=existing_params[param][0:new_shape[0]])
        else:
...
def extract_h5_paths(current_layer, next_layer):
    ...
    path_dict['wts'] = 'model_weights/' + current_layer + '/' + current_layer + '/' + 'kernel:0'
    path_dict['biases'] = 'model_weights/' + current_layer + '/' + current_layer + '/' + 'bias:0'
    path_dict['next_wts'] = 'model_weights/' + next_layer + '/' + next_layer + '/' + 'kernel:0'
    path_dict['beta'] = 'model_weights/batch_normalization_' + layer_idx + '/batch_normalization_' + layer_idx + '/' + 'beta:0'
    path_dict['gamma'] = 'model_weights/batch_normalization_' + layer_idx + '/batch_normalization_' + layer_idx + '/' + 'gamma:0'
    path_dict['moving_mean'] = 'model_weights/batch_normalization_' + layer_idx + '/batch_normalization_' + layer_idx + '/' + 'moving_mean:0'
    path_dict['moving_variance'] = 'model_weights/batch_normalization_' + layer_idx + '/batch_normalization_' + layer_idx + '/' + 'moving_variance:0'
    return path_dict
...
def extract_existing_params(oph5file, path_dict):
    ...
    params_dict['moving_variance'] = np.asarray(f[path_dict['moving_variance']])
    f.close()
    return params_dict
