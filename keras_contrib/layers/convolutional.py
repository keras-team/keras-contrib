...
class _CosineConv(Layer):
    """Abstract nD cosine convolution layer (private, used as implementation base).
    ...
    def bias_div(self, bias, k, data_format=None):
    ...
    def broadcast_mul(self, x, k, data_format=None):
    ...
    def build(self, input_shape):
        ...
        self.sum_axes = range(len(self.kernel_size) + 1)
    ...
    def call(self, inputs):
        knorm = K.sum(K.square(self.kernel), axis=self.sum_axes) + K.epsilon()
        if self.use_bias:
            b, xb = self.bias, 1.
        ...
        kxnorm = self.broadcast_mul(xnorm, knorm, self.data_format)
        outputs /= kxnorm

        if self.use_bias:
            b = self.bias_div(b, kxnorm, self.data_format)
            outputs += b
