...
def main():
    ...
    partial_gp_loss = partial(gradient_penalty_loss, trainable_weights=discriminator.trainable_weights)
    ...
    positive_y = np.array([1] * BATCH_SIZE, dtype=np.float32)
    positive_y = positive_y.reshape((-1, 1))
    negative_y = -positive_y
    dummy_y = np.array([0] * BATCH_SIZE, dtype=np.float32)
