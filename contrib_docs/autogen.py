...
# keras docs module
from docs.autogen import code_snippet
from docs.autogen import count_leading_spaces

from contrib_docs.structure import EXCLUDE
from contrib_docs.structure import PAGES
...
def post_process_signature(signature):
    ...
    if len(parts) >= 4:
        if parts[1] == 'layers':
            signature = 'keras_contrib.layers.' + '.'.join(parts[3:])
        if parts[1] == 'backend':
            signature = 'keras_contrib.backend.' + '.'.join(parts[3:])
    return signature
...
def class_to_source_link(cls):
    ...
    link = ('https://github.com/keras-team/'
            'keras-contrib/blob/master/' + path + '#L' + str(line))
    return '[[source]](' + link + ')'
...
if __name__ == '__main__':
    ...
    print('Generating docs for Keras-contrib %s.' % keras_contrib.__version__)
